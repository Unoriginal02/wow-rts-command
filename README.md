...
| **Nobody loots while the party is fighting**, nor for four seconds after — looting lives in playerbots' out-of-combat engine and it switches engines on each bot's OWN combat, so the healer nobody hits walked off mid-fight | automatic | `mod-rts/src/RtsFight.cpp` |
| **Your orders override whatever the bot had in mind** — the cast in progress is interrupted and the loot target cleared, before the order sets its own | automatic | `mod-rts/src/RtsBotApi.cpp` (`Preempt`) |
| **Dying in an instance leaves you alive at the door**, outside, with no corpse run — the spot is the core's own exit for that map | automatic, `RTS.Dungeon.ReviveAtDoor` | `mod-rts/src/RtsFight.cpp` |
| **Free the doors** — one button clears every instance lock on the party: saves, deserter, queue state, the LFG group mark (by disbanding, the core has no inverse of `ConvertToLFG`) and the finder's cooldowns | the key button, `/rts instance` | `mod-rts/src/RtsLfg.cpp` |
| **The dungeon finder works with a party of bots** — the role check is answered for them with the role you gave them (one tank and one healer at most, the rest go dps, counting your own pick), and the "ready" window is accepted for them; playerbots answered TANK for everyone (a mage included) and refused the dungeon whenever a bot was in combat | automatic, `/rts lfg` clears deserter and queue leftovers | `mod-rts/src/RtsLfg.cpp` |
